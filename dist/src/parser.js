...
function recursiveParamCollector(params, collection) {
    if (collection === void 0) { collection = {}; }
    return params.reduce(function (coll, p) {
        var _a;
        if (p.params) {
            return recursiveParamCollector(p.params, coll);
        }
        return __assign(__assign({}, coll), (_a = {}, _a[p.keyName] = p.ast, _a));
    }, collection);
}
function parseNonLiteral(schema, type, options, keyName, processed, usedNames) {
    ...
    switch (type) {
        case 'ALL_OF':
            var astParams = schema.allOf.map(function (_) { return parse(_, options, undefined, processed, usedNames); });
            var paramByKey_1 = recursiveParamCollector(astParams);
            ...
            if (Array.isArray(schema.required)) {
                ...
                var requiredParams = schema.required
                    ...
                    if (!paramByKey_1[propertyName]) {
                        // Property required that does not exist
                        return undefined;
                    }
                    return {
                        ast: paramByKey_1[propertyName],
